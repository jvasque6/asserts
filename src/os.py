...
def is_os_min_priv_enabled(server, username, password, os_type):
    """
...
def is_os_sudo_enabled(server, username, password, os_type):
    """
    Checks if there's sudo or similar installed
    """
    if os_type is 'LINUX_DEBIAN':
        os_linux_debian.is_os_sudo_enabled(server, username, password)
    elif os_type is 'LINUX_GENERIC':
        os_linux_generic.is_os_sudo_enabled(server, username, password)
    elif os_type is 'LINUX_GENTOO':
        os_linux_gentoo.is_os_sudo_enabled(server, username, password)
    elif os_type is 'LINUX_REDHAT':
        os_linux_redhat.is_os_sudo_enabled(server, username, password)
    elif os_type is 'LINUX_SLACK':
        os_linux_slack.is_os_sudo_enabled(server, username, password)
    elif os_type is 'UNIX_GENERIC':
        os_unix_generic.is_os_sudo_enabled(server, username, password)
    elif os_type is 'WINDOWS_VISTA_PLUS':
        os_windows_vista_plus.is_os_sudo_enabled(server, username,
                                                 password)
    elif os_type is 'WINDOWS_SERVER_2008_PLUS':
        os_windows_server_2008_plus.is_os_sudo_enabled(server, username,
                                                       password)


def is_os_compilers_installed(server, username, password, os_type):
    """
...
def is_os_antimalware_installed(server, username, password, os_type):
    """
    Checks if there's any antimalware installed
    """
    if os_type is 'LINUX_DEBIAN':
        os_linux_debian.is_os_antimalware_installed(server, username,
                                                    password)
    elif os_type is 'LINUX_GENERIC':
        os_linux_generic.is_os_antimalware_installed(server, username,
                                                     password)
    elif os_type is 'LINUX_GENTOO':
        os_linux_gentoo.is_os_antimalware_installed(server, username,
                                                    password)
    elif os_type is 'LINUX_REDHAT':
        os_linux_redhat.is_os_antimalware_installed(server, username,
                                                    password)
    elif os_type is 'LINUX_SLACK':
        os_linux_slack.is_os_antimalware_installed(server, username,
                                                   password)
    elif os_type is 'UNIX_GENERIC':
        os_unix_generic.is_os_antimalware_installed(server, username,
                                                    password)
    elif os_type is 'WINDOWS_VISTA_PLUS':
...
def is_os_remote_admin_enabled(server, username, password, os_type):
    """
    Checks if admins can remotely login
    """
    if os_type is 'LINUX_DEBIAN':
        os_linux_debian.is_os_remote_admin_enabled(server, username,
                                                   password)
    elif os_type is 'LINUX_GENERIC':
        os_linux_generic.is_os_remote_admin_enabled(server, username,
                                                    password)
    elif os_type is 'LINUX_GENTOO':
        os_linux_gentoo.is_os_remote_admin_enabled(server, username,
                                                   password)
    elif os_type is 'LINUX_REDHAT':
        os_linux_redhat.is_os_remote_admin_enabled(server, username,
                                                   password)
    elif os_type is 'LINUX_SLACK':
        os_linux_slack.is_os_remote_admin_enabled(server, username,
                                                  password)
    elif os_type is 'UNIX_GENERIC':
        os_unix_generic.is_os_remote_admin_enabled(server, username,
                                                   password)
    elif os_type is 'WINDOWS_VISTA_PLUS':
...
def is_os_syncookies_enabled(server, username, password, os_type):
    """
    Checks if SynCookies or similar is enabled
    """
    if os_type is 'LINUX_DEBIAN':
        os_linux_debian.is_os_syncookies_enabled(server, username,
                                                 password)
    elif os_type is 'LINUX_GENERIC':
        os_linux_generic.is_os_syncookies_enabled(server, username,
                                                  password)
    elif os_type is 'LINUX_GENTOO':
        os_linux_gentoo.is_os_syncookies_enabled(server, username,
                                                 password)
    elif os_type is 'LINUX_REDHAT':
        os_linux_redhat.is_os_syncookies_enabled(server, username,
                                                 password)
    elif os_type is 'LINUX_SLACK':
        os_linux_slack.is_os_syncookies_enabled(server, username,
                                                password)
    elif os_type is 'UNIX_GENERIC':
        os_unix_generic.is_os_syncookies_enabled(server, username,
                                                 password)
    elif os_type is 'WINDOWS_VISTA_PLUS':
