...
RUN apt-get update -qq && \
    apt-get install -y python3 python3-dev python-dev libssl-dev libffi-dev scons python-virtualenv sed curl grep gawk lsb-release && \
...
RUN apt-get update -qq && apt-get install -y docker-ce netcat

ENV FLUIDASSERTS_LICENSE_KEY=123
ENV FLUIDASSERTS_USER_EMAIL=technology@fluid.la
